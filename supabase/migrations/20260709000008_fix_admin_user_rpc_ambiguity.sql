...
BEGIN
  ...
  email_to_user AS (
    SELECT raw_email, min(raw_user_id) AS mapped_user_id
    FROM raw_sources
  ...
  SELECT
    ...
    END AS user_key,
    n.normalized_user_id AS user_id,
    min(n.raw_email) FILTER (WHERE n.raw_email IS NOT NULL) AS email,
    max(n.display_name) FILTER (WHERE n.display_name IS NOT NULL) AS display_name,
    max(n.avatar_url) FILTER (WHERE n.avatar_url IS NOT NULL) AS avatar_url,
    CASE
      WHEN bool_or(n.raw_role = 'admin') THEN 'admin'
      ELSE coalesce(max(n.raw_role) FILTER (WHERE n.raw_role IS NOT NULL), 'unknown')
    END AS role,
    max(n.raw_vip_until) AS vip_until,
    min(n.source_created_at) AS created_at,
    max(n.activity_at) AS last_activity_at,
    sum(n.visitor_event_count) AS visitor_event_count,
    sum(n.forum_post_count) AS forum_post_count,
    sum(n.forum_comment_count) AS forum_comment_count,
    sum(n.lesson_progress_count) AS lesson_progress_count,
    sum(n.attempt_count) AS attempt_count,
    bool_or(n.raw_role = 'admin') AS is_admin
  FROM normalized n
  ...
  RETURN jsonb_build_object(
    ...
    'visitor_events', COALESCE((
      ...
      FROM (
        ...
        FROM public.visitor_activity_events vae
        WHERE (v_user_id IS NOT NULL AND vae.user_id = v_user_id)
           OR (v_email IS NOT NULL AND lower(nullif(trim(coalesce(vae.email, vae.user_email)), '')) = v_email)
    ...
    'forum_posts', COALESCE((
      ...
      FROM (
        ...
        FROM public.forum_posts fp
        WHERE (v_user_id IS NOT NULL AND fp.author_user_id = v_user_id)
           OR (v_email IS NOT NULL AND lower(nullif(trim(fp.author_email), '')) = v_email)
        ORDER BY created_at DESC
        LIMIT 20
      ) x
    ), '[]'::jsonb),
    'forum_comments', COALESCE((
      ...
      FROM (
        ...
        FROM public.forum_comments fc
        WHERE (v_user_id IS NOT NULL AND fc.author_user_id = v_user_id)
           OR (v_email IS NOT NULL AND lower(nullif(trim(fc.author_email), '')) = v_email)
        ORDER BY created_at DESC
        LIMIT 20
      ) x
    ), '[]'::jsonb),
    'lesson_progress', COALESCE((
      ...
      FROM (
        ...
        FROM public.lesson_progress lp
        WHERE (v_user_id IS NOT NULL AND lp.user_id = v_user_id)
           OR (v_email IS NOT NULL AND lower(nullif(trim(lp.user_email), '')) = v_email)
