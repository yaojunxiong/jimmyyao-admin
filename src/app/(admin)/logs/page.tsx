...
const SORT_OPTIONS = [
  'created_desc', 'created_asc', 'email_asc', 'email_desc',
  'path_asc', 'path_desc', 'type_asc', 'type_desc', 'lesson_asc', 'lesson_desc',
] as const
...
function sortEvents(events: ActivityRow[], sort: string) {
  ...
  copy.sort((a, b) => {
    ...
    if (sort === 'lesson_asc') {
      const av = typeof a.lesson_no === 'number' ? a.lesson_no : Number.POSITIVE_INFINITY
      const bv = typeof b.lesson_no === 'number' ? b.lesson_no : Number.POSITIVE_INFINITY
      return av - bv
    }
    if (sort === 'lesson_desc') {
      const av = typeof a.lesson_no === 'number' ? a.lesson_no : Number.POSITIVE_INFINITY
...
function buildHref(filters: ReturnType<typeof getSearchParams>, updates: Partial<ReturnType<typeof getSearchParams>>) {
  ...
  return (
    <>
      <div className="page-header">
        <h1>Logs</h1>
        <p>Access audit log — latest 300 page visit records across the system.</p>
      </div>
        ...
        <>
          <div style={{ display: 'grid', gap: 12, gridTemplateColumns: 'repeat(auto-fit, minmax(150px, 1fr))', marginBottom: 16 }}>
            <div className="stat-card">
              <div className="stat-card-label">Filtered Results</div>
              <div className="stat-card-value">{filteredEvents.length}</div>
            </div>
            <div className="stat-card">
              <div className="stat-card-label">Signed-in Visits</div>
              <div className="stat-card-value">{signedInCount}</div>
            </div>
            <div className="stat-card">
              <div className="stat-card-label">Anonymous Visits</div>
              <div className="stat-card-value">{anonymousCount}</div>
            </div>
            <div className="stat-card">
              <div className="stat-card-label">Admin Visits</div>
          ...
          <div className="placeholder-card" style={{ marginBottom: 16 }}>
            <form method="get" style={{ display: 'grid', gap: 12, gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', alignItems: 'end' }}>
              <label style={{ display: 'grid', gap: 6 }}>
                <span style={{ fontSize: 12, fontWeight: 600, color: '#64748b' }}>Search email / path / type / UA / IP</span>
              ...
              <label style={{ display: 'grid', gap: 6 }}>
                <span style={{ fontSize: 12, fontWeight: 600, color: '#64748b' }}>Page type</span>
              ...
              <label style={{ display: 'grid', gap: 6 }}>
                <span style={{ fontSize: 12, fontWeight: 600, color: '#64748b' }}>Lesson no.</span>
              ...
              <label style={{ display: 'grid', gap: 6 }}>
                ...
                <select name="sort" defaultValue={filters.sort} style={{ border: '1px solid #cbd5e1', borderRadius: 10, padding: '9px 10px', font: 'inherit' }}>
                  <option value="created_desc">Time: newest first</option>
                  <option value="created_asc">Time: oldest first</option>
                  <option value="email_asc">User: email A-Z</option>
                  <option value="email_desc">User: email Z-A</option>
                  ...
                  <option value="lesson_asc">Lesson: ascending</option>
                  <option value="lesson_desc">Lesson: descending</option>
                </select>
              </label>
              <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                ...
                <Link href="/logs" style={{ background: 'transparent', color: '#64748b', border: '1px solid #cbd5e1', borderRadius: 10, padding: '9px 16px', fontWeight: 600, fontSize: 14, textDecoration: 'none', display: 'inline-block' }}>Clear</Link>
              </div>
            </form>
          </div>
          ...
          <div className="placeholder-card" style={{ overflowX: 'auto', padding: 0 }}>
            <h2 style={{ margin: 0, padding: '16px 16px 0', fontSize: 16, fontWeight: 700 }}>
              Recent Activity ({filteredEvents.length}/{events.length})
            </h2>
            {filteredEvents.length === 0 ? (
              <div style={{ textAlign: 'center', padding: 24 }}>
                <p style={{ color: '#94a3b8', fontSize: 14 }}>No matching activity records found.</p>
                <p style={{ color: '#94a3b8', fontSize: 11, fontFamily: 'monospace', marginTop: 8 }}>
                  DB returned {events.length} records | Filtered: {filteredEvents.length} | Range: {filters.range} | User: {filters.user} | Search: &quot;{filters.q}&quot;
                </p>
                <p style={{ marginTop: 12 }}>
                  <Link href="/logs" style={{ color: '#3b82f6', fontSize: 14 }}>Clear filters</Link>
                </p>
              </div>
            ) : (
                ...
                <thead>
                  <tr style={{ borderBottom: '2px solid #e2e8f0' }}>
                    <th style={{ padding: '10px 12px', textAlign: 'left', fontWeight: 700, color: '#475569', whiteSpace: 'nowrap' }}>
                      <Link href={buildHref(filters, { sort: nextSort(filters.sort, 'created_asc', 'created_desc') })} style={{ color: 'inherit', textDecoration: 'none' }}>
                        {sortLinkLabel(filters.sort, 'created_asc', 'created_desc', 'Time')}
                      </Link>
                    </th>
                    <th style={{ padding: '10px 12px', textAlign: 'left', fontWeight: 700, color: '#475569', whiteSpace: 'nowrap' }}>
                      <Link href={buildHref(filters, { sort: nextSort(filters.sort, 'email_asc', 'email_desc') })} style={{ color: 'inherit', textDecoration: 'none' }}>
                        {sortLinkLabel(filters.sort, 'email_asc', 'email_desc', 'User')}
                      </Link>
                    </th>
                    <th style={{ padding: '10px 12px', textAlign: 'left', fontWeight: 700, color: '#475569', whiteSpace: 'nowrap' }}>Auth</th>
                    <th style={{ padding: '10px 12px', textAlign: 'left', fontWeight: 700, color: '#475569', whiteSpace: 'nowrap' }}>
                      <Link href={buildHref(filters, { sort: nextSort(filters.sort, 'path_asc', 'path_desc') })} style={{ color: 'inherit', textDecoration: 'none' }}>
                        {sortLinkLabel(filters.sort, 'path_asc', 'path_desc', 'Path')}
                      </Link>
                    </th>
                    <th style={{ padding: '10px 12px', textAlign: 'left', fontWeight: 700, color: '#475569', whiteSpace: 'nowrap' }}>
                      <Link href={buildHref(filters, { sort: nextSort(filters.sort, 'type_asc', 'type_desc') })} style={{ color: 'inherit', textDecoration: 'none' }}>
                        {sortLinkLabel(filters.sort, 'type_asc', 'type_desc', 'Type')}
                      </Link>
                    </th>
                    <th style={{ padding: '10px 12px', textAlign: 'left', fontWeight: 700, color: '#475569', whiteSpace: 'nowrap' }}>
                      <Link href={buildHref(filters, { sort: nextSort(filters.sort, 'lesson_asc', 'lesson_desc') })} style={{ color: 'inherit', textDecoration: 'none' }}>
                        {sortLinkLabel(filters.sort, 'lesson_asc', 'lesson_desc', 'Lesson')}
                      </Link>
                    </th>
                ...
                <tbody>
                  {filteredEvents.map((event) => (
                    <tr key={event.id} style={{ borderBottom: '1px solid #f1f5f9' }}>
                      ...
                      <td style={{ padding: '8px 12px', fontSize: 11 }}>{event.page_type || '-'}</td>
                      <td style={{ padding: '8px 12px', fontSize: 11 }}>{event.lesson_no ?? '-'}</td>
                      <td style={{ padding: '8px 12px', fontFamily: 'monospace', fontSize: 11 }}>{shorten(event.ip, 15) || '-'}</td>
                      <td style={{ padding: '8px 12px', whiteSpace: 'nowrap' }}>
                        {event.workflow_instance_id ? (
                          <span style={{ fontFamily: 'monospace', fontSize: 11, color: '#3b82f6' }} title={event.workflow_instance_id}>
                            {event.workflow_instance_id.length > 8 ? event.workflow_instance_id.slice(0, 8) : event.workflow_instance_id}
                          </span>
