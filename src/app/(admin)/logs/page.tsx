...
const KNOWN_PAGE_TYPES = ['home', 'login', 'lessons', 'lesson', 'admin', 'toolbox', 'me']
const COMMON_EMAIL_STATUSES = ['sent', 'failed', 'error', 'pending']
...
function getParams(params: LogsSearchParams) {
  ...
  const pageSizeRaw = Number(params.pageSize || '50')
  const pageSize = PAGE_SIZE_OPTIONS.includes(pageSizeRaw as (typeof PAGE_SIZE_OPTIONS)[number]) ? pageSizeRaw : 50
  return { tab, q, user, range, type, lesson, activitySort, emailSort, status, template, page, pageSize }
}
...
function buildActivityHref(filters: ReturnType<typeof getParams>, updates: Partial<ReturnType<typeof getParams>>) {
  ...
  if (next.pageSize !== 50) q.set('pageSize', String(next.pageSize))
...
function PaginationControls({ filters, total, hrefBuilder }: { filters: ReturnType<typeof getParams>; total: number | null; hrefBuilder: (filters: ReturnType<typeof getParams>, updates: Partial<ReturnType<typeof getParams>>) => string }) {
  const pageCount = total === null ? null : Math.max(1, Math.ceil(total / filters.pageSize))
  const hasPrevious = filters.page > 1
  const hasNext = pageCount === null ? true : filters.page < pageCount
  const linkStyle = { border: '1px solid #cbd5e1', borderRadius: 10, padding: '8px 12px', color: '#3b82f6', fontSize: 13, fontWeight: 700, textDecoration: 'none' }
  const disabledStyle = { ...linkStyle, color: '#94a3b8', pointerEvents: 'none' as const }

  return (
    <div style={{ display: 'flex', gap: 10, alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', padding: 16, borderTop: '1px solid #f1f5f9' }}>
      <div style={{ color: '#64748b', fontSize: 13 }}>Page {filters.page}{pageCount ? ` of ${pageCount}` : ''}{total !== null ? ` | Total ${total}` : ''}</div>
      <div style={{ display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}>
        <Link href={hrefBuilder(filters, { page: filters.page - 1 })} style={hasPrevious ? linkStyle : disabledStyle}>Previous</Link>
        <Link href={hrefBuilder(filters, { page: filters.page + 1 })} style={hasNext ? linkStyle : disabledStyle}>Next</Link>
      </div>
    </div>
  )
}
...
export default async function LogsPage({ searchParams }: { searchParams: Promise<LogsSearchParams> }) {
  ...
  let emailTotal: number | null = null
  const pageFrom = (filters.page - 1) * filters.pageSize
  const pageTo = pageFrom + filters.pageSize - 1

  if (filters.tab === 'activity') {
    try {
      let query = supabase.from('visitor_activity_events').select(activitySelect, { count: 'exact' })
      const rangeStart = getRangeStart(filters.range)
      if (rangeStart) query = query.gte('created_at', new Date(rangeStart).toISOString())
      if (filters.user === 'signed-in') query = query.not('email', 'is', null)
      else if (filters.user === 'anonymous') query = query.is('email', null)
      else if (filters.user === 'admin') query = query.eq('is_admin', true)
      if (filters.type !== 'all') {
        if (filters.type === 'other') query = query.not('page_type', 'in', `(${KNOWN_PAGE_TYPES.map((type) => `"${type}"`).join(',')})`)
        else query = query.eq('page_type', filters.type)
      }
      if (filters.lesson) query = query.eq('lesson_no', filters.lesson)
      if (filters.q) {
        const escaped = filters.q.replace(/[%_\\]/g, '\\$&')
        query = query.or(`email.ilike.%${escaped}%,path.ilike.%${escaped}%,page_type.ilike.%${escaped}%,user_agent.ilike.%${escaped}%,ip.ilike.%${escaped}%,workflow_skip_reason.ilike.%${escaped}%,workflow_instance_id.ilike.%${escaped}%`)
      }
      if (filters.activitySort === 'created_asc') query = query.order('created_at', { ascending: true })
      else if (filters.activitySort === 'email_asc') query = query.order('email', { ascending: true, nullsFirst: false })
      else if (filters.activitySort === 'email_desc') query = query.order('email', { ascending: false, nullsFirst: false })
      else if (filters.activitySort === 'path_asc') query = query.order('path', { ascending: true, nullsFirst: false })
      else if (filters.activitySort === 'path_desc') query = query.order('path', { ascending: false, nullsFirst: false })
      else if (filters.activitySort === 'type_asc') query = query.order('page_type', { ascending: true, nullsFirst: false })
      else if (filters.activitySort === 'type_desc') query = query.order('page_type', { ascending: false, nullsFirst: false })
      else if (filters.activitySort === 'lesson_asc') query = query.order('lesson_no', { ascending: true, nullsFirst: false })
      else if (filters.activitySort === 'lesson_desc') query = query.order('lesson_no', { ascending: false, nullsFirst: false })
    ...
    try {
      ...
      let query = supabase.from('email_logs').select(emailLogSelect, { count: 'exact' }).order(sortColumn, { ascending, nullsFirst: false })
      const rangeStart = getRangeStart(filters.range)
      if (rangeStart) query = query.gte('created_at', new Date(rangeStart).toISOString())
  ...
  const signedInCount = events.filter((event) => !!event.email).length
  const anonymousCount = events.length - signedInCount
  const adminCount = events.filter((event) => event.is_admin).length
  const uniqueUsers = new Set(events.filter((event) => !!event.email).map((event) => event.email)).size
  const emailStatuses = Array.from(new Set([...COMMON_EMAIL_STATUSES, ...emailLogs.map((log) => log.status).filter(Boolean) as string[], filters.status !== 'all' ? filters.status : ''])).filter(Boolean).sort()
  const emailTemplates = Array.from(new Set([...emailLogs.map((log) => log.template_key || log.notification_type).filter(Boolean) as string[], filters.template !== 'all' ? filters.template : ''])).filter(Boolean).sort()
    ...
    <>
      ...
      {filters.tab === 'activity' ? (
        activityError ? (
          ...
          <>
            <div style={{ display: 'grid', gap: 12, gridTemplateColumns: 'repeat(auto-fit, minmax(150px, 1fr))', marginBottom: 16 }}>
              <div className="stat-card"><div className="stat-card-label">Total Results</div><div className="stat-card-value">{activityTotal ?? '-'}</div></div>
              <div className="stat-card"><div className="stat-card-label">Page Results</div><div className="stat-card-value">{events.length}</div></div>
              ...
              <div className="stat-card"><div className="stat-card-label">Page Unique Users</div><div className="stat-card-value">{uniqueUsers}</div></div>
            </div>
            ...
            <div className="placeholder-card" style={{ marginBottom: 16 }}>
              <form method="get" style={{ display: 'grid', gap: 12, gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', alignItems: 'end' }}>
                ...
                <label style={{ display: 'grid', gap: 6 }}><span style={{ fontSize: 12, fontWeight: 600, color: '#64748b' }}>Page size</span><select name="pageSize" defaultValue={filters.pageSize} style={{ border: '1px solid #cbd5e1', borderRadius: 10, padding: '9px 10px', font: 'inherit' }}>{PAGE_SIZE_OPTIONS.map((size) => <option key={size} value={size}>{size}</option>)}</select></label>
            ...
            <div className="placeholder-card" style={{ overflowX: 'auto', padding: 0 }}>
              <h2 style={{ margin: 0, padding: '16px 16px 0', fontSize: 16, fontWeight: 700 }}>Activity Events ({events.length}{activityTotal !== null ? `/${activityTotal}` : ''})</h2>
              {events.length === 0 ? <div style={{ textAlign: 'center', padding: 24 }}><p style={{ color: '#94a3b8', fontSize: 14 }}>No matching activity records found.</p><p style={{ marginTop: 12 }}><Link href="/logs" style={{ color: '#3b82f6', fontSize: 14 }}>Clear filters</Link></p></div> : (
                <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13, minWidth: 1200, marginTop: 12 }}>
                  <thead><tr style={{ borderBottom: '2px solid #e2e8f0' }}><th style={{ padding: '10px 12px', textAlign: 'left', color: '#475569' }}><Link href={buildActivityHref(filters, { activitySort: nextSort(filters.activitySort, 'created_asc', 'created_desc'), page: 1 })} style={{ color: 'inherit', textDecoration: 'none' }}>{sortLinkLabel(filters.activitySort, 'created_asc', 'created_desc', 'Time')}</Link></th><th style={{ padding: '10px 12px', textAlign: 'left', color: '#475569' }}>User</th><th style={{ padding: '10px 12px', textAlign: 'left', color: '#475569' }}>Auth</th><th style={{ padding: '10px 12px', textAlign: 'left', color: '#475569' }}>Path</th><th style={{ padding: '10px 12px', textAlign: 'left', color: '#475569' }}>Type</th><th style={{ padding: '10px 12px', textAlign: 'left', color: '#475569' }}>Lesson</th><th style={{ padding: '10px 12px', textAlign: 'left', color: '#475569' }}>IP</th><th style={{ padding: '10px 12px', textAlign: 'left', color: '#475569' }}>Workflow</th><th style={{ padding: '10px 12px', textAlign: 'left', color: '#475569' }}>UA</th></tr></thead>
                  <tbody>{events.map((event) => <tr key={event.id} style={{ borderBottom: '1px solid #f1f5f9' }}><td style={{ padding: '8px 12px', whiteSpace: 'nowrap', fontFamily: 'monospace', fontSize: 11 }}><Link href={`/visitors/events/${event.id}`} style={{ color: '#3b82f6', fontWeight: 700 }}>{formatTokyoDateTime(event.created_at)}</Link></td><td style={{ padding: '8px 12px', maxWidth: 160, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{event.email || <span style={{ color: '#94a3b8', fontSize: 12 }}>Anonymous</span>}</td><td style={{ padding: '8px 12px' }}>{event.is_admin ? 'Admin' : event.email ? 'User' : 'Guest'}</td><td style={{ padding: '8px 12px' }}><Link href={`/visitors/events/${event.id}`} style={{ color: '#3b82f6', textDecoration: 'none' }}><code style={{ fontSize: 11 }}>{event.path || '-'}</code></Link></td><td style={{ padding: '8px 12px', fontSize: 11 }}>{event.page_type || '-'}</td><td style={{ padding: '8px 12px', fontSize: 11 }}>{event.lesson_no ?? '-'}</td><td style={{ padding: '8px 12px', fontFamily: 'monospace', fontSize: 11 }}>{shorten(event.ip, 15) || '-'}</td><td style={{ padding: '8px 12px', whiteSpace: 'nowrap' }}>{event.workflow_instance_id ? <Link href={`/workflows/${event.workflow_instance_id}`} style={{ fontFamily: 'monospace', fontSize: 11, color: '#3b82f6' }}>{shorten(event.workflow_instance_id, 8)}</Link> : event.workflow_skip_reason || '-'}</td><td style={{ padding: '8px 12px', maxWidth: 120, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', color: '#64748b', fontSize: 12 }}>{shorten(event.user_agent, 24)}</td></tr>)}</tbody>
                </table>
              )}
              <PaginationControls filters={filters} total={activityTotal} hrefBuilder={buildActivityHref} />
            </div>
          </>
        )
      ) : (
        emailError ? (
          ...
          <>
            <div style={{ display: 'grid', gap: 12, gridTemplateColumns: 'repeat(auto-fit, minmax(150px, 1fr))', marginBottom: 16 }}>
              <div className="stat-card"><div className="stat-card-label">Total Results</div><div className="stat-card-value">{emailTotal ?? '-'}</div></div>
              <div className="stat-card"><div className="stat-card-label">Page Results</div><div className="stat-card-value">{emailLogs.length}</div></div>
            ...
            <div className="placeholder-card" style={{ overflowX: 'auto', padding: 0 }}>
              <h2 style={{ margin: 0, padding: '16px 16px 0', fontSize: 16, fontWeight: 700 }}>Email Logs ({emailLogs.length}{emailTotal !== null ? `/${emailTotal}` : ''})</h2>
              ...
              <PaginationControls filters={filters} total={emailTotal} hrefBuilder={buildEmailHref} />
            </div>
          </>
        )
      )}
